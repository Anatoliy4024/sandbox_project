...
def reserved_date(month):
    logging.info(f"Функция reserved_date вызвана для месяца: {month}")
    ...
    try:
        # Выполняем SQL-запрос, который получает все забронированные даты за месяц
        logging.info(f"Выполнение SQL-запроса для получения всех забронированных дат за месяц {month}")
        cursor.execute("""
            SELECT selected_date, COUNT(order_id) 
            FROM orders 
            WHERE status > 2 AND strftime('%Y-%m', selected_date) = ?
            GROUP BY selected_date
        """, (month,))

        # Получаем результаты запроса
        reserved_dates = cursor.fetchall()
        logging.info(f"Результат запроса: {reserved_dates}")

        # Преобразуем результат в более удобный формат
        reserved_dates_dict = {row[0]: row[1] for row in reserved_dates}
        logging.info(f"Количество забронированных заказов по датам: {reserved_dates_dict}")

        return reserved_dates_dict
