...
async def send_order_info_to_admin():
    ...
    try:
        ...
        cursor.execute(
            "SELECT order_id, user_id, session_number, language, selected_date, start_time, end_time, duration, people_count, selected_style, preferences, city, calculated_cost FROM orders WHERE user_id = ? AND status = 2 ORDER BY session_number DESC LIMIT 1",
            (user_id,)
        )
        ...
        # Шаг 4: После отправки обновляем статус в таблице users на 3001, 3002 и т.д.
